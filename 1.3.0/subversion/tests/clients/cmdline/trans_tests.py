...
def setup_working_copy(wc_dir, value_len):
  ...
  if sys.platform != 'AS/400':
    ...
    svntest.main.file_append (author_rev_unexp_path,
                              ebcdic.os400_convert_string_to_utf8("$Author$\n$Rev$"))
    svntest.main.file_append (author_rev_exp_path,
                              ebcdic.os400_convert_string_to_utf8("$Author: blah $\n$Rev: 0 $"))
    svntest.main.file_append (url_unexp_path,
                              ebcdic.os400_convert_string_to_utf8("$URL$"))
    svntest.main.file_append (url_exp_path,
                              ebcdic.os400_convert_string_to_utf8("$URL: blah $"))
    svntest.main.file_append (id_unexp_path,
                              ebcdic.os400_convert_string_to_utf8("$Id$"))
    svntest.main.file_append (id_exp_path,
                              ebcdic.os400_convert_string_to_utf8("$Id: blah $"))
    svntest.main.file_append (bogus_keywords_path,
                              ebcdic.os400_convert_string_to_utf8("$Arthur$\n$Rev0$"))
    svntest.main.file_append (embd_author_rev_unexp_path,
                              ebcdic.os400_convert_string_to_utf8("one\nfish\n$Author$ two fish\n red $Rev$\n fish"))
    svntest.main.file_append (embd_author_rev_exp_path,
                              ebcdic.os400_convert_string_to_utf8("blue $Author: blah $ fish$Rev: 0 $\nI fish"))
    svntest.main.file_append (embd_bogus_keywords_path,
                              ebcdic.os400_convert_string_to_utf8("you fish $Arthur$then\n we$Rev0$ \n\nchew fish"))
...
def keywords_from_birth(sbox):
  ...
  if sys.platform == 'AS/400':
    ...
    lines[0] = lines[0].decode('utf-8').encode('cp500')
...
def keyword_expanded_on_checkout(sbox):
  ...
  if sys.platform != 'AS/400':
    ...
    svntest.main.file_append (url_path, ebcdic.os400_convert_string_to_utf8("$URL$"))
...
def cat_keyword_expansion(sbox):
  ...
  if sys.platform != 'AS/400':
    ...
    svntest.main.file_append (mu_path, ebcdic.os400_convert_string_to_utf8("$Rev$\n$Author$"))
  ...
  if sys.platform != 'AS/400':
    ...
    svntest.actions.run_and_verify_svn (None,
                                        [ "This is the file 'mu'.\n".encode('utf-8'),
                                          ebcdic.os400_convert_string_to_utf8(
                                          "$Rev: 2 $\n"),
                                          ebcdic.os400_convert_string_to_utf8(
                                          "$Author: ")
                                          + key_author.encode('utf-8')
                                          + ebcdic.os400_convert_string_to_utf8(" $")],
                                        [],
                                        'cat', '-r', 'HEAD', mu_path)
...
def propset_commit_checkout_nocrash(sbox):
  ...
  if sys.platform != 'AS/400':
    ...
    svntest.main.file_append (mu_path,
                              ebcdic.os400_convert_string_to_utf8("$Rev$"))
  expected_output = wc.State(wc_dir, {
    'A/mu' : Item(verb='Sending'),
    })
  expected_status = svntest.actions.get_virginal_state(wc_dir, 2)
  ...
  if sys.platform != 'AS/400':
    ...
    mu_other_contents_expected = ebcdic.os400_convert_string_to_utf8(
      "This is the file 'mu'.\n$Rev: 3 $")
