...
/** Same as svn_path_join but operates on ebcdic encoded @a path and
 *  @a component, and returns an ebcdic encoded path allocated in @a pool.
 */
...
#if APR_CHARSET_EBCDIC
/** Same as svn_path_basename but operates on ebcdic encoded @a path and
 *  returns an ebcdic encoded basename allocated in @a pool.
 */
...
#if APR_CHARSET_EBCDIC
/** Same as svn_path_dirname but operates on ebcdic encoded @a path and
 *  returns an ebcdic encoded dirname allocated in @a pool..
 */
...
#if APR_CHARSET_EBCDIC
/** Same as svn_path_split but operates on ebcdic encoded @a path and
 *  creates ebcdic encoded @a *dirpath and @a *base_name, allocated in @a pool.
 */
 ...
 *  allocated in @a pool.
 *
 *  IBM's utf-ebcdic-esque multi-byte seqences in @a path are left as is.
 *  One-byte characters in @path that need escaping are escaped with the
 *  equivalent utf-8 value.
 *
 ...
 *  allocated in @a pool.
 *
 *  IBM's utf-ebcdic-esque multi-byte seqences in @a path are escaped with
 *  the equivalent utf-8 value.
 *
