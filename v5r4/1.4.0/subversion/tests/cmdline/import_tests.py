...
'''
  ...
  if sys.platform != 'AS/400':
    ...
    # On the OS400 svntest.main.file_append() doesn't open files in text mode,
    # so no line-end conversion is performed, so this line would be appended
    # with just a LF, so we explicitly make it CRLF as here.
    svntest.main.file_append(file_path, "Extra line\r\n")
