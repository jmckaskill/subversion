  ...
  while (1)
    {
      ...
      SVN_ERR (svn_stream_readline (stream, &buf, SVN_UTF8_NEWLINE_STR, &eof,
                                    pool));
      ...
      if ((buf->len >= 3)
           && (buf->data[0] == SVN_UTF8_K)
           && (buf->data[1] == SVN_UTF8_SPACE))
        {
          ...
          SVN_ERR (svn_stream_readline (stream, &buf, SVN_UTF8_NEWLINE_STR,
                                        &eof, pool));
  ...
  while (1)
    {
      ...
      if (((len == 3)
      ...
      else if ((buf[0] == SVN_UTF8_K) && (buf[1] == SVN_UTF8_SPACE))
        {
           ...
           * as a string and converting it fails since the buffer is not NULL
           * terminated. */
